...
class Reader:
    ...
    def upload_gitee(self, image_path, image_name='', ext='png'):
        ...
        date_str = str(datetime.datetime.now())[:19].replace(
            ':', '-').replace(' ', '-') + '.' + ext
...
if __name__ == '__main__':
    ...
    pdf_path = config.get('pdf_path', r'demo.pdf')
    query = config.get('query', 'all: ChatGPT robot')
    key_word = config.get('key_word', 'reinforcement learning')
    filter_keys = config.get('filter_keys', 'ChatGPT robot')
    max_results = config.get('max_results', 1)
    sort = config.get('sort', "Relevance")
    save_image = config.get('save_image', False)
    file_format = config.get('file_format', 'md')
    language = config.get('language', 'zh')

    # The rest of your code that uses these variables goes here
